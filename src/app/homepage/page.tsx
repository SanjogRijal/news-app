...
import { CNN_HEADLINES_API, TOP_HEADLINES_API } from '../components/navbar/SecondaryNav/constants';
import { useState, useEffect, useMemo } from 'react';

export default function HomepageComponent() {
    const baseUrl: string = process.env.NEXT_PUBLIC_BASE_URL as string;
    const completeURL = baseUrl + CNN_HEADLINES_API;
        ...
        return topHeadlines?.map((headline) => {
            return <Grid key={JSON.stringify(headline)} spacing={4} p={4}>
                <Grid item xs={12} sm={6} md={4}>
                    <CardComponent imageUrl={headline.urlToImage} heading={headline.title} shortContent={headline.description} fullArticle={headline.content} dateTime={headline.publishedAt} />
                </Grid>
            </Grid>
        })
    }, [topHeadlines])
